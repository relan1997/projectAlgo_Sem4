...
int main()
{
    ...
    for(int i=0;i<num_nodes;i++)
        cout<<i<<"->"<<ind_name[i]<<"->"<<people_debt[ind_name[i]]<<endl;
    map<int,vector<pair<int,int>>> flowGraph;
    for(int i=0;i<givers.size();i++)
        flowGraph[0].push_back({i+1,givers[i].second});  //inserted the edges from source to the givers nodes
    for(int i=0;i<takers.size();i++)
    {
        for(int j=0;j<givers.size();j++)
        {
            flowGraph[j+1].push_back({givers.size()+i+1,givers[j].second});  // inserted the edges between the givers and the takers
        }
    }
    for(int i=0;i<takers.size();i++)
        flowGraph[givers.size()+i+1].push_back({num_nodes-1,takers[i].second});  // inserted the edges between the takers and the sink node
    cout<<endl<<"Inserted the Edges bewteen all the nodes in the flow Graph"<<endl;
    for(auto it:flowGraph)
    {
        ...
        {
            int next_node=jt.first;
            int node_weight=jt.second;
            cout<<ind_name[curr_node]<<"->{"<<ind_name[next_node]<<" "<<node_weight<<"}";
        }
        cout<<endl;
    }
